...
// Expect variables: $TEST_BASE_URL, $TEST_LOGIN_USER, $TEST_LOGIN_PASS, $TEST_OPERATOR_ID
    if (empty($base) && !empty($TEST_BASE_URL)) {
        $base = rtrim($TEST_BASE_URL, '/');
    }
    if (empty($user) && !empty($TEST_LOGIN_USER)) {
        $user = $TEST_LOGIN_USER;
    }
    if (empty($pass) && !empty($TEST_LOGIN_PASS)) {
        $pass = $TEST_LOGIN_PASS;
    }
    if (empty($operatorId) && !empty($TEST_OPERATOR_ID)) {
...
if ($info['http_code'] !== 200) {
    echo "GET /login.php returned HTTP {$info['http_code']}\n";
    exit(2);
}
...
if ($err) {
    echo "cURL error during login: $err\n";
    exit(2);
}
...
if ($i['http_code'] !== 200) {
    echo "Login failed or index.php not accessible (HTTP {$i['http_code']})\n";
    exit(2);
}
...
if ($i['http_code'] !== 200) {
    echo "GET personnel_edit.php returned HTTP {$i['http_code']}\n";
    exit(2);
}
echo "- fetched personnel_edit for id $operatorId\n";
$csrf_edit = parseCsrf($b);
if (!$csrf_edit) {
    echo "Failed to parse CSRF token from personnel_edit page\n";
    exit(2);
}
...
function extractValue($html, $name)
{
    ...
    if (preg_match('/<textarea[^>]*name=["\']' . preg_quote($name, '/') . '["\'][^>]*>(.*?)<\/textarea>/s', $html, $m2)) {
        return $m2[1];
    }
    return '';
}
...
// treat as successful in dry-run
    ...
    if ($errSave) {
        echo "cURL error during save: $errSave\n";
        exit(2);
    }
...
// If the POST returned a redirect, follow it and assert success marker on the final page
    ...
    if ($location) {
        ...
        if ($ci['http_code'] !== 200) {
            echo "GET certification_add.php returned HTTP {$ci['http_code']}\n";
            @unlink($cookieFile);
            exit(2);
        }
        $csrf_cert = parseCsrf($cb);
        if (!$csrf_cert) {
            echo "Failed to parse CSRF token from certification_add page\n";
            @unlink($cookieFile);
            exit(2);
        }
...
// parse available certifications and trainers
        ...
        if (count($certOptions) === 0) {
            echo "No certifications available to add; skipping certification_add check.\n";
        } else {
        ...
        // trainers select is named 'completed_by'
            ...
            if ($dryRun) {
                ...
                if ($cErr) {
                        echo "cURL error during certification_save: $cErr\n";
                        @unlink($cookieFile);
                        exit(2);
                }
            }
            ...
            $cLoc = null;
            if (!empty($cHeaders) && preg_match('/^Location:\s*(.+)$/mi', $cHeaders, $mlc)) {
                $cLoc = trim($mlc[1]);
            }
            if ($cLoc) {
                if (strpos($cLoc, 'http') !== 0) {
                    $cLoc = $base . '/' . ltrim($cLoc, '/');
                }
...
// Need a CSRF token from trainer_edit.php for trainer actions
            ...
            if ($ti['http_code'] !== 200) {
                echo "GET trainer_edit.php returned HTTP {$ti['http_code']}\n";
                @unlink($cookieFile);
                exit(2);
            }
            $csrf_trainer = parseCsrf($tb);
            if (!$csrf_trainer) {
                echo "Failed to parse CSRF token from trainer_edit page\n";
                @unlink($cookieFile);
                exit(2);
            }
            ...
            if ($taErr) {
                echo "cURL error during trainer_certification_add: $taErr\n";
                @unlink($cookieFile);
                exit(2);
            }
            // follow redirect and check message
            $taLoc = null;
            if (!empty($taHeaders) && preg_match('/^Location:\s*(.+)$/mi', $taHeaders, $mta)) {
                $taLoc = trim($mta[1]);
            }
            if ($taLoc) {
                if (strpos($taLoc, 'http') !== 0) {
                    $taLoc = $base . '/' . ltrim($taLoc, '/');
                }
            ...
            if (!$csrf_trainer2) {
                echo "Failed to parse CSRF token for removal from trainer_edit page\n";
                @unlink($cookieFile);
                exit(2);
            }
            $trainerRemPost = [
            ...
            if ($trErr) {
                echo "cURL error during trainer_certification_remove: $trErr\n";
                @unlink($cookieFile);
                exit(2);
            }
            $trLoc = null;
            if (!empty($trHeaders) && preg_match('/^Location:\s*(.+)$/mi', $trHeaders, $mtr)) {
                $trLoc = trim($mtr[1]);
            }
            if ($trLoc) {
                if (strpos($trLoc, 'http') !== 0) {
                    $trLoc = $base . '/' . ltrim($trLoc, '/');
                }
